...
class DoctorEyeDropDetail extends React.Component {
  ...
  orderData() {
    orderdb.transaction((tx) => {
      tx.executeSql(
        'select * from items where patientID = 1 and eyeDropID = ?',
        [this.props.data.id],
        (_, { rows: { _array } }) => {
          if (_array.length > 0) {
            this.setState({
  ...
  updateOrder() {
    ...
    if (order !== null) {
      orderdb.transaction((tx) => {
        tx.executeSql('update items set doctorID = ? where id = ?', [
          1,
          order.id,
        ])
        tx.executeSql('update items set left = ? where id = ?', [
          left,
          order.id,
        ])
        tx.executeSql('update items set right = ? where id = ?', [
          right,
          order.id,
        ])
        tx.executeSql('update items set type = ? where id = ?', [
          isAbnormal,
          order.id,
        ])
      })
        ...
        tx.executeSql(
          ...
          (_, { rows: { _array } }) => {
            _array.forEach((time) => {
              ...
              this.scheduleNotification(order.id, time.time).then(
                (notificationId) => {
                  timedb.transaction((rx) => {
                    rx.executeSql(
                      'update items set notificationID = ? where id = ?',
                      [notificationId, time.id],
                    )
                  })
                },
              )
  ...
  addTime() {
    ...
    if (order === null) {
      orderdb.transaction((tx) => {
        tx.executeSql(
          'insert into items (patientID, doctorID, eyeDropID, start, end, left, right, type) values (?,?,?,?,?,?,?,?)',
          [
            1,
            1,
            this.props.data.id,
          ...
          async (_, { insertId }) => {
            ...
            timedb.transaction((rx) => {
              rx.executeSql(
                'insert into items (orderID, time, notificationID) values (?,?,?)',
                [insertId, date, notificationId],
                () => this.orderData(),
              )
            })
          },
      ...
      if (select === null) {
        ...
        if (duplicate()) {
          ...
          this.scheduleNotification(order.id, date).then((notificationId) => {
            timedb.transaction((tx) => {
              tx.executeSql(
                'update items set time = ? where id = ?',
                [date, select.id],
                tx.executeSql(
                  'update items set notificationID = ? where id = ?',
                  [notificationId, select.id],
                  () => this.orderData(),
                ),
              )
            })
  ...
  renderTimeList(item) {
    return (
      <Button
        onPress={() =>
          this.setState({ date: item.time, select: item, visible: true })
        }
    ...
    return (
      <View style={{ flex: 1 }}>
        <CardImage title={data.name} source={{ uri: data.image }}>
          <Card>{this.renderDetailList(data.detail.split(','))}</Card>
        </CardImage>
        <Card>
          <Row>
            <ButtonSmallText
